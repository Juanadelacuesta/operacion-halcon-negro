...
The `home-routes` handler in the `operacion-halcon-negro.routes.home` namespace
...
The `home-page` function will in turn call the `operacion-halcon-negro.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `operacion-halcon-negro.handler` namespace:
...
Request middleware functions are located under the `operacion-halcon-negro.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `operacion-halcon-negro.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `operacion-halcon-negro.db.core` in some other namespace.
